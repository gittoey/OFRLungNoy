...
FROM
	...
	systemconfig
	ON 
		orderdetail.GradeCode = systemconfig.ConfigValue AND
		systemconfig.ConfigCode = 'Grade'
WHERE `orderdetail`.`Active` = TRUE AND `orderdetail`.`OrderID` = {$_POST["OrderID"]}
";

$sql .= " ORDER BY `orderdetail`.`OrderDetailID` ASC";
...
$_POST["Return"]["SysOrderDetail"] = $dataReturn;
